export const metadata = {
  title: "The evidence | Which Food Is Killing You?",
  description:
    "Every claim in the film, with its source and its status — consensus, contested, or the director's own synthesis. Published so anyone can check it.",
};

type Status = "Consensus" | "Contested" | "Synthesis";

const CLAIMS: { time: string; claim: string; status: Status; source: string }[] = [
  /* ---------------- PART ONE — WHAT FOOD IS  (0–17) ---------------- */
  {
    time: "01:00",
  ...
  },
  {
    ...
    status: "Contested",
    source: "See the fuller entry at 41:00. One of the most actively disputed questions in nutrition science; the film argues one side of it.",
  },
  {
    time: "04:00",
  ...
  /* ---------------- PART TWO — HOW WE GOT HERE  (17–33) ---------------- */
  {
    time: "20:00",
...
export default function EvidencePage() {
  ...
  return (
    <main style={{ background: "#0f0f0f", color: "#e8e2d5", minHeight: "100vh" }}>
      ...
      <header style={{
        ...
        <div style={wrap}>
          ...
          <p style={{ fontSize: "18px", lineHeight: 1.65, opacity: .82, maxWidth: "58ch", margin: 0 }}>
            The film makes {CLAIMS.length} substantive claims. All of them are here, with a
            source, and marked according to how well the evidence actually supports them.
          </p>
        </div>
      </header>
        ...
        <div style={wrap}>
          ...
          <div style={{
            ...
            <Key
              label="Consensus"
              colour="#7fa87f"
              n={counts.Consensus ?? 0}
              text="Settled scientific agreement. If you looked it up you would find the same thing."
            />
            <Key
              label="Contested"
              colour="#d4af37"
              n={counts.Contested ?? 0}
              text="The literature genuinely disagrees, and the film argues one side of it. Some of these note where the film goes further than its source."
            />
            <Key
              label="Synthesis"
              colour="#c98b5e"
              n={counts.Synthesis ?? 0}
              text="The director's own inference. Not published, not peer-reviewed, and not presented as established."
            />
          </div>

          <p style={{ ...para, maxWidth: "62ch" }}>
            Most documentaries state everything with the same confidence. This one does not,
            because the difference between a settled finding and an argument is the actual
            subject of the film.
          </p>

          <p style={{ ...para, maxWidth: "62ch" }}>
            Where the film overstates something, the note says so. There are a few. They are
            marked rather than quietly dropped, because a table that only confirmed what the
            film says would be worth nothing.
          </p>
          ...
          <div style={{
            ...
            <p style={{ ...para, marginBottom: 0 }}>
              <strong style={{ color: "#d4af37" }}>Corrections are welcome</strong> and will be
              published here. If something is wrong, or a source does not say what the note
              claims it says, write to{" "}
      ...
      {/* THE TABLE */}
      <section id="table" style={{ ...section, background: "#141414", paddingTop: "clamp(40px,5vw,56px)" }}>
        <div style={wrap}>
      ...
      <section style={{ ...section, paddingBottom: "clamp(72px,10vw,120px)" }}>
        <div style={wrap}>
          <h2 style={h2}>If you want to go further</h2>

          <p style={{ ...para, maxWidth: "62ch" }}>
            The two rows marked Synthesis are the film&apos;s own arguments &mdash; the
            combination mechanism, and the Ireland thesis. Neither has been published or
            tested. They are the reason the film exists and the parts most likely to be
            wrong.
          </p>

          <p style={{ ...para, maxWidth: "62ch" }}>
            If you are a researcher, a clinician or simply somebody who reads this
            literature properly, I would rather hear from you than not.
          </p>

          <p style={{ ...para, fontSize: "17px" }}>
            <a href="mailto:colin@poshpork.com" style={link}>colin@poshpork.com</a>
          </p>

          <p style={{ ...para, fontSize: "15px", opacity: .7, maxWidth: "62ch", marginTop: "30px" }}>
            Writing about the film? Everything else &mdash; stills, synopses, how it was
            made &mdash; is on the <a href="/press" style={link}>press page</a>.
          </p>
          ...
          <div style={{
            ...
            <p style={{ ...para, fontSize: "15px", marginBottom: 0, opacity: .8 }}>
              ...
              and nothing in it should replace a conversation with your doctor. Nobody should
              change prescribed medication on the strength of a film.
            </p>
          </div>
        </div>
      </section>
...
function Key({ label, colour, n, text }: { label: string; colour: string; n: number; text: string }) {
  return (
    <div style={{ borderTop: `2px solid ${colour}`, paddingTop: "16px" }}>
      <p style={{
        display: "flex",
        alignItems: "baseline",
        gap: "10px",
      ...
      <p style={{ fontSize: "14px", lineHeight: 1.6, color: "#a8a29a", margin: 0 }}>{text}</p>
    </div>
  );
}
